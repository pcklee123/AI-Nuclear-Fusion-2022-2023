...
void kernel tnp_k_optimized(
    global const float8 *a1, global const float8 *a2,     // E, B coeff
    global float *x0, global float *y0, global float *z0, // initial pos
    global float *x1, global float *y1, global float *z1, const float Bcoeff,
    const float Ecoeff,                            // Bcoeff, Ecoeff
  ...
  for (int t = 0; t < ncalc; t++) {
    ...
    uint idx = ((uint)((z - ZLOW) / DZ) * NZ + (uint)((y - YLOW) / DY)) * NY +
               (uint)((x - XLOW) / DX);
    ...
    // float r_det = 1.0 / (r1*(r2 + xxP + yyP + zzP));
    ...
    x = clamp(xE + r_det *
                       ((1.f + xxP) * dx + (zP + xyP) * dy + (xzP - yP) * dz),
              XLOW, XHIGH);
    y = clamp(yE + r_det *
                       ((xyP - zP) * dx + (1.f + yyP) * dy + (yzP + xP) * dz),
              YLOW, YHIGH);
    z = clamp(zE + r_det *
                       ((xzP + yP) * dx + (yzP - xP) * dy + (1.f + zzP) * dz),
              ZLOW, ZHIGH);
    // if(id == 0) printf("%1.15e -> %1.15e, %1.15e -> %1.15e, %1.15e ->
    // %1.15e\n", xprev, x, yprev, y, zprev, z); if(id == 0) printf("d:%1.15e,
    // %1.15e, %1.15e\ns:%1.15e, %1.15e, %1.15e\n", x-xprev, y-yprev, z-zprev,
    // xE, yE, zE); if(id == 0) printf("%s %s %s\n",
    // sign(x-xprev)==sign(xE)?"true ":"false",sign(y-yprev)==sign(yE)?"true
    // ":"false",sign(z-zprev)==sign(zE)?"true ":"false");
...
void kernel tnp_k_implicitbak(
  ...
  for (int t = 0; t < ncalc; t++) {
    ...
    // float vxnext = b_det * ((b_prime - 2 * (yyP + zzP)) * vx + (2 * (zP +
    // xyP)) * vy + (2 * (xzP - yP)) * vz + (1.f + xxP) * xE + (zP + xyP) * yE +
    // (xzP - yP) * zE); float vynext = b_det * ((2 * (xyP - zP)) * vx +
    // (b_prime - 2 * (xxP + zzP)) * vy + (2 * (xP + yzP)) * vz + (xyP - zP) *
    // xE + (1.f + yyP) * yE + (yzP + xP) * zE); float vznext = b_det * ((2 *
    // (yP + xzP)) * vx + (2 * (yzP - xP)) * vy + (b_prime - 2 * (xxP + yyP)) *
    // vz + (xzP + yP) * xE + (yzP - xP) * yE + (1.f + zzP) * zE);

    // Collect common terms
    // float vxxe = 2 * vx + xE,
    //       vyye = 2 * vy + yE,
    //       vzze = 2 * vz + zE;
    // float vxnext = vx + b_det * (-2 * vx * (yyP + zzP) + vyye * (zP + xyP) +
    // vzze * (xzP - yP) + (1.f + xxP) * xE); float vynext = vy + b_det * (vxxe
    // * (xyP - zP) + -2 * vy * (xxP + zzP) + vzze * (xP + yzP) + (1.f + yyP) *
    // yE); float vznext = vz + b_det * (vxxe * (yP + xzP) + vyye * (yzP - xP) +
    // -2 * vz * (xxP + yyP) + (1.f + zzP) * zE);
